...
//       } else {
//         router.replace('/teacher-pending');  // NEW: Add this line
//       }
...
const HomePage = () => {
  ...
  // UPDATED: Early redirection for teachers/super-admins, even before isReady
  useEffect(() => {
    if (!isAuthLoading && profile) {
      if (profile.role === 'teacher') {
        ...
        return;  // Exit early, no need for further logic
      } else if (profile.role === 'super-admin') {
        router.replace('/super-admin');
        return;  // Exit early
      }
      // For students, proceed
      setIsReady(true);
    }
  }, [profile, isAuthLoading, router]);
  ...
  // UPDATED: Show loader only if loading or no profile; for teachers/super-admins, don't show content
  if (isAuthLoading || !profile) {
    return <FullPageLoader />;
  }

  // If we're here, it must be a student (due to early return above)
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        {/* Header */}
